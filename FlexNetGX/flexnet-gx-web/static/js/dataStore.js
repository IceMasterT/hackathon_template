...
const DataStore = {
  ...
  addPendingUser: function(user) {
    this.data.pendingUsers.push(user);
    this.saveData(this.data);
  },

  updateUser: function(updatedUser) {
    const index = this.data.users.findIndex(user => user.username === updatedUser.username);
    if (index !== -1) {
  ...
  assignRole: function(username, role) {
    const userIndex = this.data.pendingUsers.findIndex(user => user.username === username);
    if (userIndex !== -1) {
      const user = this.data.pendingUsers.splice(userIndex, 1)[0];
      user.role = role;
      this.data.users.push(user);
      this.saveData(this.data);
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function handleLogin(e) {
    ...
    // Check user data
    const userData = DataStore.getData().users;
